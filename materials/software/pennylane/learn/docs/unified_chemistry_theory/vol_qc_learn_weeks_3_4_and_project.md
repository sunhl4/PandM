...
> **生成**：`tools/build_unified_qc_learn_weeks.py`。**编辑请改源文件**（`learning/classical-chem/QC-learn/week3_classical_ml.md`；`learning/quantum-chem/learning-ms/week4_quantum_ml.md`、`final_project_ideas.md`），再重跑本脚本。

> **说明**：与「电子结构传统理论」大稿见仓库路径 `learning/classical-chem/QC-learn/quantum_chemistry_foundations.md`（未并入本卷，避免单文件过大）。
