...
![分裂价：两个径向尺度与线性组合示意](../assets-figure/orbitals/fig_radial_split_valence.png)
...
![极化：纯 s 与叠加 pz 的定性对比](../assets-figure/orbitals/fig_polarization_s_pz.png)
...
![s、p、d、f 各一例（三维角向）](../assets-figure/orbitals/fig_ao3d_s_p_d_f_samples.png)

**$p$（简并度 3）**：

![px py pz 三维](../assets-figure/orbitals/fig_ao3d_p_all.png)

**$d$（简并度 5）**：

![五个实 d 三维](../assets-figure/orbitals/fig_ao3d_d_all.png)

**$f$（简并度 7）**：

![七个实 f 三维](../assets-figure/orbitals/fig_ao3d_f_all.png)
...
![s 与 p 轨道示意](../assets-figure/orbitals/fig_ao_s_p.png)
...
![d 轨道示意](../assets-figure/orbitals/fig_ao_d.png)
...
![f 轨道示意（混合截面，见上文）](../assets-figure/orbitals/fig_ao_f.png)
...
![sp 与 sp2 杂化截面示意](../assets-figure/orbitals/fig_hybrids_sp_sp2.png)

![sp3 之一瓣在 xz 对角线方向](../assets-figure/orbitals/fig_hybrid_sp3_lobe.png)
...
输出目录：`assets-figure/orbitals/`（二维与三维 PNG 均在此目录）。
...
本部分按照“先建立方法直觉，再进入严格推导，最后落实到矩阵算法与 SCF 迭代”的顺序展开。**去重约定**：**§6.1** 侧重路线图、物理直觉与定理陈述；**Slater 行列式、$E_{\mathrm{HF}}$ 逐项推导、Lagrange 乘子到 Fock 方程的泛函导数** 的完整细节集中在 **§6.2**；**§6.3** 在给出与程序一致的 **Slater–Condon 记号** 收束后，从 **LCAO / Roothaan–Hall / SCF** 接续（避免同一推导链重复三遍）。
...
#### 3.2 Fock 算符与 Roothaan–Hall 方程（提要）

闭壳层 RHF 在 AO 基下最终归结为 **广义本征方程** $\mathbf{F}\mathbf{C}=\mathbf{S}\mathbf{C}\boldsymbol{\varepsilon}$。**$\hat{J}_j,\hat{K}_j$ 的积分定义**、**“平均 Coulomb + 非局域交换”** 的物理解读、**$F_{\mu\nu}$ 用 $(\mu\nu|\lambda\sigma)$ 与密度矩阵 $\mathbf{P}$ 的显式展开**，见 **§6.2.4.3 之五、之六** 与 **§6.3 §7–§8**（与量子化学程序接口完全一致）。

#### 3.3 SCF 迭代（提要）

$\mathbf{F}=\mathbf{F}[\mathbf{P}]$ 而 $\mathbf{P}=\mathbf{P}[\mathbf{C}]$，故为 **不动点迭代**：初猜 $\mathbf{P}^{(0)}\to$ 构造 $\mathbf{F}^{(k)}\to$ 解 $\mathbf{F}\mathbf{C}=\mathbf{S}\mathbf{C}\boldsymbol{\varepsilon}\to$ 更新 $\mathbf{P}^{(k+1)}\to$ 判敛；**DIIS** 等外推见 **§6.3 §9**。**流程图（Mermaid）** 亦置于 **§6.3 §9**，与本节避免重复。
...
**一句话**：在 **Hartree–Fock 单行列式** 框架下，若从 **占据** 自旋轨道上移去一个电子且 **假定其余分子轨道保持不变（冻结轨道近似）**，则 **电离能** 近似等于该轨道能量 **相反数**：

$$
\mathrm{IP}_i \equiv E(N{-}1)-E(N) \approx -\varepsilon_i^{\mathrm{(occ)}}.
$$

更常见的写法是 $\varepsilon_i \approx -\mathrm{IP}_i$（$\varepsilon_i$ 为 HF **正则轨道能**，即 Roothaan–Hall 广义本征值 $\mathbf{FC}=\mathbf{SC}\boldsymbol{\varepsilon}$ 的对角元）。

**推导要点（直观）**：设闭壳层 RHF 参考 $|\Phi_0\rangle$，从占据空间轨道 $\psi_i$ 取走一个电子得到 **单空穴** 态 $|\Phi_i\rangle$（其余电子仍占据原来的 HF 轨道）。若 **不允许** 其余轨道弛豫，则 $|\Phi_i\rangle$ 的能量相对 $|\Phi_0\rangle$ 的变化在一阶上由 **Fock 算符对该轨道的期望值** 给出，即 $\varepsilon_i$；于是 $\Delta E = E_i - E_0 \approx -\varepsilon_i$，即 $\mathrm{IP}_i \approx -\varepsilon_i$。**弛豫**（离子态下其余轨道重新优化以降低能量）会使真实 $\mathrm{IP}_i$ 与 $-\varepsilon_i$ 产生偏差；**电子相关**（HF 未包含的动态/静态相关）进一步修正电离过程的能量。

**成立条件与适用范围**：

- **占据轨道**：对 **闭壳层 RHF**，深价壳与内壳价轨道常给出 **定性合理** 的 $\mathrm{IP}$ 趋势；对 **前线轨道**（HOMO 等），弛豫与相关往往更明显，数值上 $-\varepsilon_{\mathrm{HOMO}}$ 与实验 $\mathrm{IP}$ 可差 **数 eV** 仍属常见。
- **开壳层 UHF/ROHF**：需指明 **移去 $\alpha$ 还是 $\beta$ 电子** 与哪一套自旋轨道能；原则上仍可按“冻结自旋轨道 + 单空穴”类比，但实际解释更需谨慎（自旋污染、对称性简并等）。
- **虚轨道（未占据）**：常把 **电子亲和** 粗写成 $\mathrm{EA} \approx -\varepsilon_a^{\mathrm{virt}}$。由于虚轨道 **非真实的中性体系占据态**，弛豫与相关误差往往 **比电离情形更大**，通常 **只做数量级或趋势讨论**，不宜当作高精度预测。

**小结**：Koopmans 定理把 **HF 正则轨道能** 与 **电离能** 联系起来，是解释光电子能谱与前线轨道概念的 **定性桥梁**；定量计算 $\mathrm{IP}$/$\mathrm{EA}$ 应使用 **ΔSCF**（分别优化中性体与离子）、格林函数、耦合簇/EOM 等方法。
...
**本节定位**：全部分第三部分中 **反对称性 $\to$ Slater 行列式 $\to$ $E_{\mathrm{HF}}[\{\phi_i\}]$ 的 $J/K$ 分解（含两电子展开）$\to$ Lagrange 乘子与泛函导数 $\to$ 正则 Fock 方程与 SCF 思想** 的 **主推导链**；读 §6.1 仅建立鸟瞰的读者，应在本节补全证明与算符细节。
...
**为什么用指数算符？**（简要）数学上：分离系统时 $e^{\hat{T}_A + \hat{T}_B} = e^{\hat{T}_A} e^{\hat{T}_B}$，自动满足大小一致性；指数展开用少量振幅（$\hat{T}$ 的系数）自动生成高激发（如 $\hat{T}_2^2$ 给出四激发），参数少而表达能力强。物理上：指数形式对应“连接关联”的乘积结构，符合多体理论中能量与波函数可分解的图像；$e^{\hat{T}}$ 可理解为对参考态的“完全相关化”。**CC 的系统推导、振幅方程与大小一致性证明** 见 **第四部分**（后 Hartree-Fock / 耦合簇章节），不在本节重复展开。
...
下文 **§1–§4** 给出与 **§6.2** 并行的 **Slater–Condon 记号**、闭壳层 $h,J,K$ 的 **显式空间积分**（便于与下文 Mulliken $(\mu\nu|\lambda\sigma)$ 收缩），以及 **轨道能与总能** 的常用恒等式；**不重述** Lagrange 泛函与 $\delta/\delta\phi_k^*$ 的逐步推导（**见 §6.2.4.3**）。**§6–§9** 给出 LCAO–Roothaan 矩阵形式与 SCF 算法；**§10** 起为 UHF 与常用定理的简表。

---

#### 1. Born–Oppenheimer 下的电子哈密顿量与 HF 变分表述
...
**HF 近似**：在反对称性约束下，将 $N$ 体波函数限制为 **单个 Slater 行列式** $\Phi(\mathbf{x}_1,\ldots,\mathbf{x}_N)=\frac{1}{\sqrt{N!}}\det[\psi_i(\mathbf{x}_j)]$（**正交归一自旋轨道**），并在该约束下 **变分求能量极小**。行列式的构造、反对称与归一化的 **初等证明**、以及 Hartree 积与 Slater 行列式的对比，见 **§6.2.4.1**。

---

#### 2. Slater–Condon 规则：单行列式能量的紧凑形式（一般自旋轨道）

对归一化单行列式 $\Phi$，能量 $E = \langle\Phi\vert\hat{H}_{\mathrm{elec}}\vert\Phi\rangle$ 可用 **Slater–Condon 规则** 写出。对 **正交归一自旋轨道**，最紧凑的形式为
...
这里 $\langle ij\vert ij\rangle$ 表示按空间–自旋坐标积分的 **非反对称** Coulomb 型矩阵元（与 **§0** 中 Mulliken **空间** 积分类似，但此处仍带自旋坐标）。

---

#### 3. 闭壳层 RHF：$E_{\mathrm{RHF}}$ 与 $h_{ii},J_{ij},K_{ij}$（与 §6.2.4.2 等价）

闭壳层下，占据自旋轨道成对出现：对每个空间轨道 $\phi_i$ 有 $\psi_{i\alpha}=\phi_i\alpha$、$\psi_{i\beta}=\phi_i\beta$。将上式对自旋求和，得到仅用 **空间轨道** 的表达；其与 **§6.2.4.2** 中从两电子 Slater 行列式 **逐项展开** 得到的 $J_{ij},K_{ij}$ 定义 **代数相同**（同一能量泛函的两种书写习惯）。
...
#### 4. 驻点条件、正则 Fock 方程与轨道能（证明见 §6.2.4.3）

HF 问题：在 $\langle\phi_i|\phi_j\rangle=\delta_{ij}$ 约束下最小化 $E_{\mathrm{RHF}}[\{\phi_i\}]$。**Lagrange 泛函** $\mathcal{L}=E_{\mathrm{RHF}}-\sum_{ij}\varepsilon_{ij}(\langle\phi_i|\phi_j\rangle-\delta_{ij})$、对 $\phi_k^*$ 的 **泛函导数**、以及 **$\varepsilon_{ij}$ 对角化** 得到正则轨道的 **完整逐步推导**，见 **§6.2.4.3**（含 $\hat{J}_j,\hat{K}_j$ 从能量项的导出）。闭壳层 **收束** 为耦合形式与正则形式：
...
\qquad
\hat{f}\,\phi_i = \varepsilon_i\,\phi_i\ \ \text{（正则表象）}.
$$

$\hat{f}$ 非线性，需 **SCF**（**§9**）。由 $\langle \phi_i\vert\hat{f}\vert\phi_i\rangle = \varepsilon_i$ 得

$$
\varepsilon_i = h_{ii} + \sum_j (2J_{ij} - K_{ij}).
$$

总能量 **不等于** $\sum_i 2\varepsilon_i$（否则会双重计算电子–电子相互作用）：
...
（第二项与 $E=2\sum h_{ii}+\sum(2J-K)$ 为代数恒等。）

**占据子空间的酉不变性**：在同一 **占据子空间** 内对 $\{\phi_i\}$ 做酉变换，Slater 行列式至多差 **整体相位**，$E_{\mathrm{RHF}}$ 不变；正则轨道 $\hat{f}\phi_i=\varepsilon_i\phi_i$ 是使 $\varepsilon_{ij}$ **对角化** 的便利表象，虚轨道在虚子空间内亦酉等价。
...
**Brillouin** 与 **Koopmans** 的陈述、MP2 衔接及 Koopmans 的 **推导要点、适用条件与定量局限** 已集中在 **§6.1「3.6 重要定理」**（含 Koopmans 专节），此处不重复。

---

#### 12. 计算复杂度小结

与 **§6.1.3.7** 同一量级结论；下表从 **AO 基数 $K$** 强调实现视角（与 §6.1 中有时用分子尺度 $N$ 的粗记法并存，二者在文献中常互换使用，需注意定义）。
...
| §6.1 总览（3.x） | 理论详推 / 矩阵与算法 |
| :----------------: | :------------------------ |
| 3.1 平均场 + 单行列式 | **§6.2.4.1**（Slater）；**§6.3 §1–§2**（BO + Slater–Condon） |
| 3.2 Fock 与 Roothaan（提要） | **§6.2.4.3**（$\hat{f}$ 导出）；**§6.3 §4**（收束式）；**§7–§8**（矩阵元） |
| 3.3 SCF（提要） | **§9**（逐步 + Mermaid）；**§6.2.4.3 之六**（SCF 思想连续形式） |
| 3.4 RHF/UHF/ROHF | **§10**（UHF）；**§6.1.3.4**（表） |
| 3.5–3.7 物理、定理与标度 | **§6.1.3.5–3.7**（定理含 Koopmans）；标度另见 **§12** |

更完整的 **CI/CC/DFT** 脉络见 **第四部分起**。
...
##### 4.4.1 Davidson 校正（+Q）：动机、公式与限度

**名称澄清**：量子化学里常把两件事都叫 “Davidson”——**(i) Davidson 对角化/子空间迭代**：求大稀疏 $\mathbf{H}$ 少数本征对时的 **算法**（见前文维度/稀疏性讨论）；**(ii) Davidson（+Q）能量校正**：对 **截断 CI 能量** 的 **后验修正**，与对角化算法 **无必然绑定**（只是历史上常与 Davidson 对角化一起出现在 CI 程序中）。本节专指 **(ii)**。

**它试图修补什么？** 对 **单参考 CISD**，变分空间含参考、单激发、**连通**双激发，但 **不含** 一般四激发。而在 **无相互作用** 的 $A\cdots B$ 上，有一部分 **四激发可分解为**「$A$ 上双激发」$\otimes$「$B$ 上双激发」的 **不连通（disconnected）** 乘积；这类行列式 **不在** CISD 流形内，却 **对** $E(A{+}B)$ **的加和性** 有贡献，于是出现 **非大小一致性（lack of size consistency）**：$E_{\mathrm{CISD}}(A{+}B)\neq E_{\mathrm{CISD}}(A)+E_{\mathrm{CISD}}(B)$。直观上，CISD **少算了** 一批「看起来像两个独立双激发叠在一起」的相关效应；**+Q** 用参考权重与 CISD 相关能构造一个 **低代价** 的 **经验/微扰型** 补丁，**部分**追回这部分缺失。

**\(c_0\) 是什么？** 将归一化 CISD 波函数写为

$$
|\Psi_{\mathrm{CISD}}\rangle = c_0|\Phi_0\rangle + \sum_{ia} c_i^a|\Phi_i^a\rangle + \sum_{i<j,a<b} c_{ij}^{ab}|\Phi_{ij}^{ab}\rangle,
$$

其中 $|\Phi_0\rangle$ 为（通常未再归一化到 CI 内部的）HF 参考行列式，**\(c_0\)** 为其 **展开系数**。归一化下

$$
|c_0|^2 + \sum_{ia}|c_i^a|^2 + \sum_{ijab}|c_{ij}^{ab}|^2 = 1,
$$

故 **\(1-c_0^2\)**（实轨与实系数时常略写模方）度量 **激发组态总权重**：参考越「纯」，\(c_0^2\) 越接近 1，截断越温和；**静态相关** 强时 \(c_0^2\) 变小，意味着 CISD 本身已可能 **不可靠**，此时 **不应** 指望 +Q 单独解决问题。

**最常用的单参考 CISD+Q 写法（Pople / Davidson 型经验公式）** 定义

$$
E_{\mathrm{corr}}^{\mathrm{(CISD)}} \equiv E_{\mathrm{CISD}} - E_{\mathrm{HF}},\qquad
\Delta E_Q \equiv (1-c_0^2)\,E_{\mathrm{corr}}^{\mathrm{(CISD)}},
$$
...
= E_{\mathrm{HF}} + \bigl(2-c_0^2\bigr)\,E_{\mathrm{corr}}^{\mathrm{(CISD)}}.
$$

即 **相关能** 从 $E_{\mathrm{corr}}^{\mathrm{(CISD)}}$ 放大为 $\bigl(2-c_0^2\bigr)E_{\mathrm{corr}}^{\mathrm{(CISD)}}$（在 $c_0^2<1$ 时 **大于** 原始 CISD 相关能），体现对 **缺失的类四激发/不连通双×双** 份额的 **外推补偿**；当 $c_0^2\to 1$ 时因子 $\to 1$，+Q 项趋于零。其理论脉络来自对 **缺失高阶激发** 的 **Rayleigh–Schrödinger / Epstein–Nesbet** 型分析（Langhoff–Davidson 等早期工作）；在实用上常把它视为 **对 CISD 系统偏差的经验补丁**。

**能补到什么程度？** +Q **改善** 许多弱关联体系上 **二聚体加和性、解离曲线** 等表现，但 **一般不能** 把 CISD 变成 **严格** size-consistent 方法；**结构性地** 保证 **大小延展性（size extensivity）** 的是 **耦合簇** 的指数 ansatz（以及正确截断下的 FCI）。当 **\(c_0^2\)** 已经很小（多参考特征显著）时，上式可能给出 **过大** 的修正，应改用 **多参考 CI / MRCI+Q**（**另一套** 针对 MR-SDCI 的归一化公式，如 Knowles–Handy 形式；**勿**与上式单参考 CISD+Q 混用）或 **CASSCF+NEVPT2/CASPT2** 等。

**小结**：**Davidson（+Q）** 用 **\((1-c_0^2)\)** 与 **CISD 相关能** 估计截断 CI 因 **缺失不连通四激发类** 而损失的能量；**实现零额外迭代成本**（只需 CI 向量），适合 **单参考主导** 的 CISD；**不是** 万能的大小一致性修复，也 **不是** Davidson 对角化算法本身。
...
本节从 **Rayleigh–Schrödinger 微扰论（RSPT）** 的一般公式出发，再落实到 **Møller–Plesset（MP）划分**，推出 **MP2 能量** 的闭合式，并交代 **高阶、Goldstone 图与收敛性**。**MP0—MP4 与程序输出对照** 集中在 **§5.8**；与 **§8.2** 的编排关系见该章首段说明（**MP 不再在 §8.2 单列一节**）。

##### 5.1 Rayleigh–Schrödinger 微扰论：通式

设全哈密顿依赖形式参数 $\lambda$：

$$
\hat{H}(\lambda) = \hat{H}_0 + \lambda\hat{V},\qquad \lambda=1\ \text{对应真实体系}.
$$

假定 **零阶** $\hat{H}_0$ 在参考子空间上可对角化，**归一化** 零阶波函数 $|\Psi^{(0)}\rangle\equiv|0\rangle$ 满足

$$
\hat{H}_0|0\rangle = E^{(0)}|0\rangle,
$$

并设 $\{|n\rangle\}_{n\neq 0}$ 为 $\hat{H}_0$ 的其余本征态，$\hat{H}_0|n\rangle=E_n^{(0)}|n\rangle$，$\langle m|n\rangle=\delta_{mn}$。将 **精确** 解按 $\lambda$ 泰勒展开，取 **中间归一化** $\langle 0|\Psi(\lambda)\rangle=1$（并记 $|\Psi^{(0)}\rangle\equiv|0\rangle$）。以下 **两种记号** 在教材与程序注释中常见，**彼此等价**：

$$
|\Psi(\lambda)\rangle = |0\rangle + \lambda|\Psi^{(1)}\rangle + \lambda^2|\Psi^{(2)}\rangle + \cdots,\qquad
E(\lambda) = E^{(0)} + \lambda E^{(1)} + \lambda^2 E^{(2)} + \cdots .
$$

$$
E = E^{(0)}+\lambda E^{(1)}+\lambda^2 E^{(2)}+\cdots,\qquad
|\Psi\rangle = |\Psi^{(0)}\rangle+\lambda|\Psi^{(1)}\rangle+\lambda^2|\Psi^{(2)}\rangle+\cdots,
\quad \langle\Psi^{(0)}|\Psi\rangle=1 .
$$

代入 $\hat{H}(\lambda)|\Psi\rangle=E|\Psi\rangle$，逐阶收集 $\lambda$ 的同次幂，得 **RSPT 标准结果**（至二阶）：

$$
E^{(1)} = \langle 0|\hat{V}|0\rangle,
$$

$$
|\Psi^{(1)}\rangle = \sum_{n\neq 0} \frac{\langle n|\hat{V}|0\rangle}{E^{(0)}-E_n^{(0)}}\,|n\rangle,
$$

$$
E^{(2)} = \sum_{n\neq 0} \frac{|\langle n|\hat{V}|0\rangle|^2}{E^{(0)}-E_n^{(0)}} = \langle 0|\hat{V}|\Psi^{(1)}\rangle .
$$

**三阶能量** $E^{(3)}$ 由 $\hat{V}$ 的三重连接与 **$E^{(1)}$ 对二阶波函数的反馈** 共同构成，显式双重求和式较长；标准推导见 Helgaker–Jørgensen–Olsen 或 Szabo–Ostlund，程序实现多用 **Goldstone 图 / 自动求导** 而非手写全部项。

##### 5.2 Møller–Plesset 划分：$\hat{H}_0$、$\hat{V}$ 与「HF 作为零阶」

取 **HF 正则自旋轨道** 与同一套 **Fock 算符** $\hat{f}$（第三部分记号）。对 $N$ 电子定义

$$
\hat{H}_0 \equiv \sum_{\mu=1}^{N} \hat{f}(\mu),\qquad
\hat{V} \equiv \hat{H} - \hat{H}_0 .
$$

**零阶本征值**：HF 行列式 $|\Phi_0\rangle$ 满足

$$
\hat{H}_0|\Phi_0\rangle = \Bigl(\sum_{i\in\mathrm{occ}}\varepsilon_i\Bigr)|\Phi_0\rangle,
\qquad
E^{(0)} = \sum_{i\in\mathrm{occ}}\varepsilon_i
$$

（求和遍及 **占据自旋轨道**；闭壳层 RHF 常写为 $2\sum_{i=1}^{N/2}\varepsilon_i$，$\varepsilon_i$ 为 **空间** 轨道能）。

**一阶能量与 HF**：

$$
E^{(1)} = \langle\Phi_0|\hat{V}|\Phi_0\rangle
= \langle\Phi_0|\hat{H}|\Phi_0\rangle - \langle\Phi_0|\hat{H}_0|\Phi_0\rangle
= E_{\mathrm{HF}} - E^{(0)},
$$
...
$$
E^{(0)} + E^{(1)} = E_{\mathrm{HF}}.
$$

即 **到一阶为止微扰重现 Hartree–Fock**；**二阶起** $E^{(2)},E^{(3)},\ldots$ 才给出 **纯相关** 修正。$\hat{V}$ 称为 **涨落势（fluctuation potential）**：双电子排斥相对 Fock 平均场的 **残差**。

**双激发的零阶能量差**：在正则轨道下，双激发 $|\Phi_{ij}^{ab}\rangle$ 相对参考的 **零阶本征值抬升** 为

$$
\langle\Phi_{ij}^{ab}|\hat{H}_0|\Phi_{ij}^{ab}\rangle - E^{(0)}
= \varepsilon_a + \varepsilon_b - \varepsilon_i - \varepsilon_j,
$$

此即 MP 分母中 **单粒子激发能** 的来源（具体自旋/闭壳因子与求和范围与程序约定一致时需统一）。

##### 5.3 Brillouin 定理与一阶波函数：单激发系数为零

对单激发 $|\Phi_i^a\rangle$，**Brillouin 定理**给出 $\langle\Phi_i^a|\hat{H}|\Phi_0\rangle=0$；又 $\langle\Phi_i^a|\hat{H}_0|\Phi_0\rangle=0$，故

$$
\langle\Phi_i^a|\hat{V}|\Phi_0\rangle = 0.
$$

代入 $|\Psi^{(1)}\rangle$ 的 RS 公式，**所有单激发系数** 为零。**二阶能量** 中首个一般非零通道因而来自 **双激发**（$\hat{V}$ 为二体算符，与参考差 **两个** 占据轨道的行列式才可通过 Slater–Condon 与 $\hat{V}$ 耦合）。

##### 5.4 二阶 MP2：矩阵元、分母与闭合式

**矩阵元**（Slater–Condon）：

$$
\langle\Phi_{ij}^{ab}|\hat{V}|\Phi_0\rangle = \langle ij\Vert ab\rangle,
$$

其中 $\langle ij\Vert ab\rangle$ 为 **反对称化双电子积分**（记号与程序接口一致即可；chemist 与 physicist 记号转换需注意）。

**分母**：$E^{(0)}-E_{ijab}^{(0)} = -(\varepsilon_a+\varepsilon_b-\varepsilon_i-\varepsilon_j)$。代入 $E^{(2)}$ 求和得 **闭壳层常见写法**：

$$
E_{\mathrm{MP2}} - E_{\mathrm{HF}} = E^{(2)}
= -\sum_{i<j}^{\mathrm{occ}}\sum_{a<b}^{\mathrm{vir}}
\frac{|\langle ij\Vert ab\rangle|^2}{\varepsilon_a + \varepsilon_b - \varepsilon_i - \varepsilon_j}.
$$
...
**仅差** 在分母提出负号；**UHF** 需按 $\alpha/\beta$ 块分别取轨道能与积分。

**物理解释**：$E^{(2)}<0$ 体现 **二阶稳定化**；分母中出现 **小能隙** $\varepsilon_a+\varepsilon_b-\varepsilon_i-\varepsilon_j$ 时项发散 → **MP 级数不可靠**。

##### 5.5 高阶 MP$n$、Goldstone 图与 EN 划分（提要）

- **MP3 / MP4**：三阶起出现 **单–双–三** 链与更复杂拓扑；**MP4(SDTQ)** 显式收进至 **四激发** 的部分通道，标度分别约 **$O(N^6)$、$O(N^7)$**。
- **Goldstone 相连图定理**：MP 能量可写为 **相连 Goldstone 图** 之和，保证适当截断下的 **大小延展性**（与 §4.4 截断 CI 对比）。
- **Epstein–Nesbet**：若把分母换为 **HF 哈密顿下** 的真实激发能 $\Delta E_{0\to n}^{\mathrm{EN}}$，得到 **EN 划分**；与标准 **MP 分母** 收敛域不同，强相关时行为可差异很大。
...
**图 5-1**：电子对 $(i,j)$ 在涨落势下 **协同** 激发至 $(a,b)$；**MP2 是相关能的第一个非零微扰阶**（因 $E^{(1)}$ 已并入 $E_{\mathrm{HF}}$）。

**关键要点**：

- **标度**：**$O(N^5)$**（或 $O(K^5)$ 视角），远廉于 CCSD。
- **精度经验**：弱关联闭壳层有机物上，MP2 对 **相关能** 常给出 **约 80–90%** 量级的恢复（不宜与 CCSD(T) 的绝对化学精度直接类比）。
- **适用/不适用**：能隙小、多参考、UHF 自旋污染严重时慎用；可配合 **SCS-MP2 / SOS-MP2** 等 **自旋分量缩放** 改善某些相互作用。

##### 5.7 MP3、MP4 与级数收敛性

**MP3 / MP4** 的 **图论骨架**、与 **MP2 的衔接** 及 **EN 划分** 等提要见 **§5.5**；**各阶标度与「说明」式总览** 并入 **§5.8** 下表，以免与 **§5.7/§5.8** 重复排两张仅差数列的表。

**收敛性问题**：MP 展开 **不保证收敛**；强关联或坏参考时高阶项可 **振荡或发散**，应转向 **多参考、CC 或 DMRG** 等。

##### 5.8 各阶 MP$n$ 能量与标度一览（程序输出对照）

下表将 **MP0—MP4** 的 **能量关系**、**计算标度** 及 **§5.5、§5.7** 中对 MP3/MP4 的文字要点 **收束在一处**（**MP2 的显式求和式** 仍以 **§5.4** 为准；**MP3/MP4 的图论结构** 仍以 **§5.5** 为准）。表中 $E^{(k)}$ 为 RSPT/Møller–Plesset 展开系数，$\lambda=1$ 时总能量为 $E_{\mathrm{MP}n}=\sum_{k=0}^{n} E^{(k)}$（记号与 Gaussian、PySCF 等常见输出一致时需核对是否打印的是 **累计** 还是 **本级** 修正）。

| 阶 | 能量关系（要点） | 标度（粗，电子数/基组视角） |
| :--: | :-- | :-- |
| **MP0** | $E^{(0)}=\sum_{i\in\mathrm{occ}}\varepsilon_i$（占据 **自旋** 轨道能之和；**单独**不是物理总能量） | 已由 HF 本征问题给出 |
| **MP1** | $E^{(0)}+E^{(1)}=E_{\mathrm{HF}}$，$E^{(1)}=\langle\Phi_0|\hat{V}|\Phi_0\rangle$ | 同 HF |
| **MP2** | $E_{\mathrm{MP2}}-E_{\mathrm{HF}}=E^{(2)}$；$E^{(2)}$ 为 **双激发** 求和，见 **§5.4** | **$O(N^5)$**（或 $O(K^5)$） |
| **MP3** | $E^{(3)}$ 含单–双–三链；显式式长，见 **§5.5** 与专著；**性价比**：修正常不大 | **$O(N^6)$** |
| **MP4** | **MP4(SDTQ)** 等方案收进至 **四激发** 部分通道；**精度**常明显高于 MP2 | **$O(N^7)$** |
...
| **MRCI**（Multi-Reference CI）                                     | 多参考 + SD 激发            | 最系统但标度高；常用 **MRCI+Q**（多参考 Davidson 型能量校正，Knowles–Handy 等实现，**公式不同于** 单参考 **CISD+Q**，见 **§8.1 §4.4.1**）缓解 size-intensive 误差            |
...
### 8.2 数学框架细讲：CI 与 CC（MP 见 §8.1 §5）

**编排说明**：**Møller–Plesset（MP）多体微扰** 的 RSPT 通式、$\hat{H}_0/\hat{V}$ 划分、Brillouin、MP2 闭合式、图解、高阶提要、**MP0—MP4 一览与收敛性** 已全部写在 **§8.1「§5 多体微扰理论」**；本节 **6.1–6.2** 仅展开 **CI** 与 **CC** 的矩阵/振幅框架，**不再** 重复 MP 公式。
...
```mermaid
flowchart LR
    Vext["外势 v_ext"] <--> Rho["基态密度 rho"]
    Rho <--> Psi["基态波函数 Psi"]
    Psi --- Etot["总能量与可观测量"]
```

**图 8-1**：Hohenberg–Kohn 第一定理的映射关系：外势、基态密度、基态波函数与能量之间的逻辑链条（示意）。
...
**图 8-2**：Kohn–Sham DFT 的自洽场（SCF）迭代流程（示意）。
...
John Perdew提出的"雅各布天梯"比喻：从"凡间"（简单近似）到"天堂"（化学精度）。下图概括五阶依赖的信息量（与下文 LDA→双杂化 分节一一对应）。

```mermaid
flowchart BT
    S1["LDA 局域密度"]
    S2["GGA 含密度梯度"]
    S3["meta-GGA 含动能密度 tau"]
    S4["Hybrid 含部分精确交换"]
    S5["Double-hybrid 含 MP2 型相关"]
    S1 --> S2 --> S3 --> S4 --> S5
```

**图 9-1**：Jacob's ladder（雅各布天梯）示意图。
...
| **meta-GGA**      | SCAN, M06-L  | 引入动能密度，精度常优于 GGA     |
| **Hybrid**        | B3LYP, PBE0  | 混合 HF 交换，有机化学常用       |
| **Double-Hybrid** | B2PLYP, XYG3 | 含 MP2 型相关，精度可接近 CCSD(T) |
...
5. **激发态**：
   ...
   - 基态 DFT 只求基态密度与能量
   - 激发态常用 **TDDFT**（含时 DFT，常写成 Casida 线性响应形式）；电荷转移、Rydberg 等类型需谨慎，见 **§7.5**
6. **弱相互作用**：

   - 标准局域或半局域泛函难以描述长程色散（范德华力）
   - 常用 **Grimme DFT-D3/D4** 等色散校正项；总能量常拆写为 $E_{\mathrm{total}} = E_{\mathrm{DFT}} + E_{\mathrm{disp}}$，对 $\pi$–$\pi$ 堆积、分子间作用、生物大分子构象尤为重要
...
在泛函与基组选定之后，可按问题类型做快速取舍；下表与 **§7.3**「如何选择泛函」互补（更偏场景与理由）。

##### 场景与泛函建议

| 场景               | 推荐泛函         | 理由                           |
| :------------------- | :----------------- | :------------------------------- |
| **有机反应**       | ωB97X-D, M06-2X | 色散与高比例交换，势垒常更可靠 |
| **大规模固体**     | PBE, SCAN        | 效率与结构描述的综合平衡       |
| **激发态（TDDFT）** | CAM-B3LYP 等     | 范围分离泛函常改善电荷转移激发 |
| **过渡金属配合物** | PBE0, TPSSh 等   | 杂化有助于缓解离域与 SIE 问题   |

##### TDDFT（含时 DFT）与 Casida 方程

**含时密度泛函理论（TDDFT）** 在激发能、UV–vis 谱等任务中最为常用。线性响应下常归结为 **Casida 方程**：
...
由此得到激发能 $\omega$ 与振子强度等；具体实现因程序对 Tamm–Dancoff 近似（TDA）与全耦合形式的选择而异。

```mermaid
flowchart TD
    GS["基态密度 rho_0"] -->|"含时微扰 v t"| TD["含时密度 rho t"]
    TD -->|"线性响应"| EX["激发能与光谱"]
```

**图 10-1**：TDDFT 的物理逻辑（示意）：由基态出发，对外场含时扰动做响应，经线性响应得到激发态信息。
...
```mermaid
flowchart TD
    Start["方法选择"] --> Main["主族化学为主"]
    Main -->|是| SingleRef["单参考是否足够"]
    Main -->|否| Metal["过渡金属化学为主"]
    SingleRef -->|是| Quick["快速或大体系"]
    SingleRef -->|否| MultiRef["多参考 CASSCF 等"]
    Quick -->|是| DFTn["DFT 如 wB97X-D"]
    Quick -->|否| Gold["CCSD(T) 与 CBS"]
    Metal -->|是| DFTU["DFT 加 U 或杂化"]
    Metal -->|否| SolidQ["固态或周期体系为主"]
    SolidQ -->|是| PBEn["PBE 或 SCAN"]
```
